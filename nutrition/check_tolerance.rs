...
struct Input { deviation: DeviationResult, tolerance_pct: f64 }
...
fn main() -> io::Result<()> {
    ...
    let mut v = Vec::new();
    if d.protein_pct.abs() > t { v.push(format!("protein: {:.1}%", d.protein_pct)); }
    if d.fat_pct.abs() > t { v.push(format!("fat: {:.1}%", d.fat_pct)); }
    if d.carbs_pct.abs() > t { v.push(format!("carbs: {:.1}%", d.carbs_pct)); }
    let r = ToleranceCheckResult {
        within_tolerance: v.is_empty(),
        max_deviation: d.protein_pct.abs().max(d.fat_pct.abs()).max(d.carbs_pct.abs()),
        violations: v,
    };
