...
# Primary test variables (used by Gleam integration tests)
TANDOOR_URL=$TANDOOR_URL
TANDOOR_USERNAME=$TANDOOR_USER
TANDOOR_PASSWORD=$TANDOOR_PASS

# Legacy test variables (for backward compatibility)
...
EOF
    ...
    if [ -n "$token" ]; then
        echo "" >> "$ENV_TEST_FILE"
        echo "# Bearer token (alternative to session auth)" >> "$ENV_TEST_FILE"
