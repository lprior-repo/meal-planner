/**
 * FatSecret Food Brands API documentation scraper
 */
...
/**
 * Scraper for FatSecret Food Brands API documentation endpoints
 */
export class BrandsScraper extends BaseScraper {
  private readonly targetEndpoints = ['brands.get.v2', 'brand.search.v1'];

  /**
   * Navigate to Food Brands category and extract endpoint links
   */
  async navigateToCategory(): Promise<PageLink[]> {
    ...
    // Navigate to the Food Brands API guides page
    ...
    const links = await this.page.evaluate(() => {
      const linkElements = Array.from(
        (document as any).querySelectorAll('a[href*="brands"], a[href*="brand"]')
      );
      return linkElements
        .map((el: any) => ({
  ...
  async saveOutput(endpointName: string, markdown: string, jsonExample: unknown): Promise<void> {
    const mdDir = join(this.config.outputPath, 'markdown', 'food_brands');
    const jsonDir = join(this.config.outputPath, 'fixtures', 'food_brands');
  ...
  private formatEndpointMarkdown(
    ...
    // Document brand types enumeration
    markdown += `### Brand Types\n\n`;
    markdown += `The \`brand_type\` parameter accepts the following values:\n\n`;
    markdown += `- **Manufacturer** - Food manufacturers and producers\n`;
    markdown += `- **Restaurant** - Restaurant chains and food service brands\n`;
    markdown += `- **Supermarket** - Retail grocery and supermarket brands\n\n`;
  ...
  async run(): Promise<void> {
    try {
      ...
      // Navigate to Food Brands category and get links
...
/**
 * Convenience function to run the brands scraper
 */
export async function scrapeBrandsDocs(): Promise<void> {
  const scraper = new BrandsScraper({
