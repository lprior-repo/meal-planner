...
pub fn basic_card_renders_multiple_content_test() {
  ...
  |> should.equal("<div class=\"card\"><h2>Title</h2><p>First</p><p>Second</p></div>")
}
...
pub fn card_with_header_renders_correctly_test() {
  ...
  |> should.equal("<div class=\"card\"><div class=\"card-header\">My Header</div><div class=\"card-body\"><p>Content</p></div></div>")
}

pub fn card_with_header_handles_empty_content_test() {
  card.card_with_header("Header Only", [])
  |> should.equal("<div class=\"card\"><div class=\"card-header\">Header Only</div><div class=\"card-body\"></div></div>")
}
...
pub fn card_with_actions_renders_all_elements_test() {
  let result = card.card_with_actions("Task Card", ["<p>Task description</p>"], ["<button>Edit</button>", "<button>Delete</button>"])
...
pub fn stat_card_renders_all_fields_test() {
  let stat = ui_types.StatCard(label: "Calories", value: "2100", unit: "kcal", trend: option.Some(5.0), color: "#4CAF50")
...
pub fn stat_card_handles_no_trend_test() {
  let stat = ui_types.StatCard(label: "Weight", value: "75.5", unit: "kg", trend: option.None, color: "#2196F3")
  let result = card.stat_card(stat)
...
pub fn recipe_card_with_image_renders_test() {
  let recipe = ui_types.RecipeCardData(id: "recipe-001", name: "Grilled Chicken", category: "Main Course", calories: 450.5, image_url: option.Some("https://example.com/chicken.jpg"))
  let result = card.recipe_card(recipe)
  
  result |> string.contains("recipe-card") |> should.be_true
  result |> string.contains("<img src=\"https://example.com/chicken.jpg\" />") |> should.be_true
...
pub fn recipe_card_without_image_renders_test() {
  let recipe = ui_types.RecipeCardData(id: "recipe-002", name: "Garden Salad", category: "Salad", calories: 125.0, image_url: option.None)
  let result = card.recipe_card(recipe)
...
pub fn food_card_renders_all_fields_test() {
  let food = ui_types.FoodCardData(fdc_id: 123456, description: "Chicken, broilers or fryers, breast, meat only, raw", data_type: "Survey (FNDDS)", category: "Poultry Products")
  let result = card.food_card(food)
  
  result |> string.contains("food-card") |> should.be_true
  result |> string.contains("Chicken, broilers or fryers, breast, meat only, raw") |> should.be_true
...
pub fn cards_handle_empty_strings_test() {
  ...
  result1 |> string.contains("<div class=\"card-header\"></div>") |> should.be_true
...
pub fn stat_card_handles_zero_values_test() {
  let stat = ui_types.StatCard(label: "Zero", value: "0", unit: "items", trend: option.None, color: "#000000")
...
pub fn recipe_card_handles_zero_calories_test() {
  let recipe = ui_types.RecipeCardData(id: "r0", name: "Water", category: "Beverages", calories: 0.0, image_url: option.None)
...
pub fn cards_have_proper_class_structure_test() {
  let result1 = card.card(["content"])
  result1 |> string.contains("class=\"card\"") |> should.be_true
  
  let result2 = card.card_with_header("h", ["c"])
  ...
  let result3 = card.card_with_actions("h", ["c"], ["a"])
