    ...
    Ok(conn) -> {
      ...
      let results =
        list.map(recipes, fn(recipe) {
          case recipes.save_recipe(conn, recipe) {
            Ok(_) -> {
      ...
      let beef_count = case recipes.get_recipes_by_category(conn, "beef-main") {
        Ok(r) -> list.length(r)
        Error(_) -> 0
      }

      let bison_count = case
        recipes.get_recipes_by_category(conn, "bison-main")
      {
        Ok(r) -> list.length(r)
        Error(_) -> 0
      }

      let lamb_count = case recipes.get_recipes_by_category(conn, "lamb-main") {
        Ok(r) -> list.length(r)
        Error(_) -> 0
      }

      let rice_count = case recipes.get_recipes_by_category(conn, "rice-side") {
        Ok(r) -> list.length(r)
        Error(_) -> 0
      }

      let veg_count = case
        recipes.get_recipes_by_category(conn, "vegetable-side")
      {
        Ok(r) -> list.length(r)
        Error(_) -> 0
      }
