...
///
/// These tests demonstrate the complete workflow with Tandoor recipes:
/// 1. Create a food log entry with a recipe slug
/// 2. Validate the recipe exists in Tandoor before saving
...
import meal_planner/storage/recipe_validation.{RecipeNotFound, error_to_message}
...
pub fn construct_tandoor_food_log_entry_test() {
  let entry =
    FoodLogEntry(
...
pub fn typical_user_workflow_test() {
  ...
  // 3. Entry should have Tandoor source type
  entry.source_type
  |> should.equal("tandoor_recipe")
}
...
pub fn common_tandoor_recipe_slugs_test() {
  let common_recipes = [
    "chicken-breast",
    "salmon-fillet",
    "broccoli-florets",
...
pub fn graceful_error_handling_test() {
  let invalid_slug = "this-recipe-does-not-exist-in-tandoor"
