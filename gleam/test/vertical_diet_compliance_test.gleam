...
pub fn no_red_meat_detected_test() {
  ...
  list.any(result.reasons, fn(r) {
    string.contains(r, "No red meat")
  })
  |> should.be_true()
}
...
pub fn no_simple_carbs_detected_test() {
  ...
  list.any(result.reasons, fn(r) {
    string.contains(r, "No simple carbs")
  })
  |> should.be_true()
}
...
pub fn complex_ingredients_nine_count_test() {
  ...
  list.any(result.reasons, fn(r) {
    string.contains(r, "Complex recipe")
  })
  |> should.be_true()
}
...
pub fn very_long_recipe_name_test() {
  ...
  let recipe = create_recipe(long_name, None, [create_ingredient("salt")], [create_instruction("Cook")], None)
...
pub fn special_characters_in_ingredients_test() {
  let recipe = create_recipe(
    "Recipe",
    None,
    [create_ingredient("grass-fed beef (organic)"), create_ingredient("white rice [premium]")],
    [create_instruction("Cook")],
    None,
  )
