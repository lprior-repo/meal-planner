...
pub fn all_returned_match_protein_constraint_test() {
  ...
  let filtered =
    all
    |> list.filter(fn(m) { matches_protein_range(m, min_protein, max_protein) })
...
pub fn filtered_count_lte_total_test() {
  ...
  let filtered =
    all |> list.filter(fn(m) { matches_protein_range(m, Some(30.0), None) })
...
pub fn no_constraints_returns_all_test() {
  ...
  let filtered =
    all
    |> list.filter(fn(m) {
      matches_protein_range(m, None, None)
      && matches_fat_range(m, None, None)
      && matches_carbs_range(m, None, None)
    })
...
pub fn tighter_protein_constraint_reduces_results_test() {
  let all = test_macros()
  let loose =
    all |> list.filter(fn(m) { matches_protein_range(m, Some(10.0), None) })
  let tight =
    all |> list.filter(fn(m) { matches_protein_range(m, Some(30.0), None) })
...
pub fn combining_constraints_reduces_results_test() {
  let all = test_macros()
  let single =
    all |> list.filter(fn(m) { matches_protein_range(m, Some(20.0), None) })
  let double =
    all
    |> list.filter(fn(m) {
      matches_protein_range(m, Some(20.0), None)
      && matches_fat_range(m, None, Some(20.0))
    })
...
pub fn minimum_boundary_inclusive_test() {
  let all = test_macros()
  let filtered =
    all |> list.filter(fn(m) { matches_protein_range(m, Some(50.0), None) })
  // Should include high_protein with exactly 50.0g protein
  let has_50_protein =
    filtered |> list.any(fn(m) { m.protein >=. 49.9 && m.protein <=. 50.1 })
...
pub fn maximum_boundary_inclusive_test() {
  let all = test_macros()
  let filtered =
    all |> list.filter(fn(m) { matches_protein_range(m, None, Some(50.0)) })
  // Should include high_protein with exactly 50.0g protein
  let has_50_protein =
    filtered |> list.any(fn(m) { m.protein >=. 49.9 && m.protein <=. 50.1 })
  has_50_protein |> should.be_true
...
pub fn protein_filter_independent_test() {
  ...
  let filtered =
    all |> list.filter(fn(m) { matches_protein_range(m, min_protein, None) })
  // All should have protein >= 25.0
  filtered
  |> list.each(fn(m) { { m.protein >=. 25.0 } |> should.be_true })
}
...
pub fn fat_filter_independent_test() {
  ...
  let filtered =
    all |> list.filter(fn(m) { matches_fat_range(m, None, max_fat) })
  // All should have fat <= 15.0
  filtered
  |> list.each(fn(m) { { m.fat <=. 15.0 } |> should.be_true })
}
...
pub fn carbs_filter_independent_test() {
  let all = test_macros()
  let filtered =
    all |> list.filter(fn(m) { matches_carbs_range(m, Some(15.0), Some(55.0)) })
...
pub fn idempotent_filtering_test() {
  ...
  let once =
    all |> list.filter(fn(m) { matches_protein_range(m, Some(20.0), None) })
  let twice =
    once |> list.filter(fn(m) { matches_protein_range(m, Some(20.0), None) })
...
pub fn contradictory_constraints_empty_test() {
  ...
  let filtered =
    all
    |> list.filter(fn(m) { matches_protein_range(m, Some(100.0), Some(50.0)) })
...
pub fn exact_match_test() {
  ...
  let filtered =
    all
    |> list.filter(fn(m) { matches_protein_range(m, Some(30.0), Some(30.0)) })
  // Should only match balanced with exactly 30.0g protein
  let count_30 =
    filtered
    |> list.filter(fn(m) { m.protein >=. 29.9 && m.protein <=. 30.1 })
    |> list.length
...
pub fn no_matching_excluded_test() {
  let all = test_macros()
  let should_match =
    all |> list.filter(fn(m) { m.protein >=. 25.0 && m.protein <=. 50.0 })
  let filtered =
    all
    |> list.filter(fn(m) { matches_protein_range(m, Some(25.0), Some(50.0)) })
