//! Integration tests for FatSecret Most Eaten foods binary
//!
//! Tests: fatsecret_foods_most_eaten
...
fn test_fatsecret_foods_most_eaten_invalid_page() {
    ...
    let result = run_binary("fatsecret_foods_most_eaten", &input);
    assert!(result.is_ok(), "Binary should handle invalid page gracefully");
}
