...
struct Input { history: Vec<NutritionState>, goals: NutritionGoals, tolerance_pct: f64 }

fn within(goals: &NutritionGoals, s: &NutritionState, tol: f64) -> bool {
    let dev = |g: f64, a: f64| if g == 0.0 { 0.0 } else { ((a - g) / g).abs() * 100.0 };
...
fn main() -> io::Result<()> {
    ...
    let within_count = i.history.iter().filter(|s| within(&i.goals, s, i.tolerance_pct)).count();
    let r = ConsistencyResult {
        consistency_rate: if total == 0 { 0.0 } else { (within_count as f64 / total as f64) * 100.0 },
