...
struct Output { compliance_score: f64, protein_score: f64, fat_score: f64, carbs_score: f64, calories_score: f64, grade: String }

fn score(g: f64, a: f64) -> f64 { if g == 0.0 { 100.0 } else { (100.0 - ((a - g) / g).abs() * 100.0).max(0.0) } }
fn grade(s: f64) -> &'static str { match s as u32 { 90..=100 => "A", 80..=89 => "B", 70..=79 => "C", 60..=69 => "D", _ => "F" } }
...
fn main() -> io::Result<()> {
    ...
    let r = Output { compliance_score: total, protein_score: ps, fat_score: fs, carbs_score: cs, calories_score: cals, grade: grade(total).to_string() };
