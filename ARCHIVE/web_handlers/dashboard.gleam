...
pub fn handle(req: wisp.Request, conn: pog.Connection) -> wisp.Response {
  ...
  let date =
  case list.key_find(wisp.get_query(req), "date") {
    Ok(d) -> d
...
fn render_date_selector(date: String) -> String {
  "<div class=\"date-nav\">
    <button hx-get=\"/api/dashboard/data?date=" <> prev_date(date) <> "\"
            hx-target=\".grid\"
            hx-swap=\"innerHTML\">
      ← Previous Day
    </button>
    <input type=\"date\"
           ...
           hx-swap=\"innerHTML\"
           hx-include=\"this\">
    <button hx-get=\"/api/dashboard/data?date=" <> next_date(date) <> "\"
            hx-target=\".grid\"
            hx-swap=\"innerHTML\">
      Next Day →
    </button>
...
fn render_daily_content(
  ...
  <!-- Daily Log Card (partial - no recent foods in update) -->
  " <> render_daily_log(daily_log)
}
...
fn prev_date(date: String) -> String {
  // Simple date math - in production, use proper date library
...
fn next_date(date: String) -> String {
  case date {
