...
use crate::fatsecret::recipes::types::{
    Recipe, RecipeId, RecipeResponseWrapper, RecipeSearchResponse, RecipeSearchResponseWrapper,
    RecipeAutocompleteResponseWrapper, RecipeSuggestion, RecipeType, RecipeTypesResponseWrapper,
};
...
pub async fn get_recipe(
    ...
    let response: RecipeResponseWrapper = serde_json::from_str(&body)
        .map_err(|e| FatSecretError::ParseError(format!("Failed to parse recipe: {}. Body: {}", e, body)))?;
...
pub async fn search_recipes(
    ...
    let mut params = HashMap::new();
    params.insert("search_expression".to_string(), search_expression.to_string());
    ...
    let response: RecipeSearchResponseWrapper = serde_json::from_str(&body)
        .map_err(|e| FatSecretError::ParseError(format!("Failed to parse recipe search: {}. Body: {}", e, body)))?;
...
pub async fn autocomplete_recipes(
    ...
    let response: RecipeAutocompleteResponseWrapper = serde_json::from_str(&body)
        .map_err(|e| FatSecretError::ParseError(format!("Failed to parse recipe autocomplete: {}. Body: {}", e, body)))?;

    Ok(response.suggestions.suggestions)
}

/// Get all recipe types (recipe_types.get.v2 - 2-legged)
pub async fn get_recipe_types(
    config: &FatSecretConfig,
) -> Result<Vec<RecipeType>, FatSecretError> {
    let params = HashMap::new();

    let body = make_api_request(config, "recipe_types.get.v2", params).await?;
    let response: RecipeTypesResponseWrapper = serde_json::from_str(&body)
        .map_err(|e| FatSecretError::ParseError(format!("Failed to parse recipe types: {}. Body: {}", e, body)))?;
