//! Core error types for application
//!
...
pub enum Error {
    ...
    ExternalApi { message: String, service: String },
    
    /// Resource not found error
    #[error("Resource not found: {message}")]
    NotFound { message: String, resource_type: String },
...
impl Error {
    ...
    pub fn http(message: impl Into<String>, status_code: u16) -> Self {
        Self::Http { message: message.into(), status_code }
    }
    ...
    pub fn external_api(message: impl Into<String>, service: impl Into<String>) -> Self {
        Self::ExternalApi { message: message.into(), service: service.into() }
    }
    
    /// Create a new not found error
    pub fn not_found(message: impl Into<String>, resource_type: impl Into<String>) -> Self {
        Self::NotFound { message: message.into(), resource_type: resource_type.into() }
    }
    ...
    /// Get's error status code (for HTTP responses)
...
impl From<serde_json::Error> for Error {
    fn from(error: serde_json::Error) -> Self {
        Self::Internal { message: format!("JSON serialization error: {}", error) }
    }
}

impl From<reqwest::Error> for Error {
    fn from(error: reqwest::Error) -> Self {
        Self::ExternalApi { message: format!("External API error: {}", error), service: "external".to_string() }
    }
}

impl From<clap::Error> for Error {
    fn from(error: clap::Error) -> Self {
        Self::Validation { message: format!("CLI argument error: {}", error), field: None }
    }
}

impl From<uuid::Error> for Error {
    fn from(error: uuid::Error) -> Self {
        Self::Validation { message: format!("UUID error: {}", error), field: None }
    }
}

impl From<chrono::ParseError> for Error {
    fn from(error: chrono::ParseError) -> Self {
        Self::Validation { message: format!("Date parsing error: {}", error), field: None }
    }
}
