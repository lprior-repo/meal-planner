...
pub fn execute_get(
  ...
  use req <- result.try(request_builder.build_get_request(config, path, query_params))
  logger.debug("Tandoor GET " <> path)
  use resp <- result.try(execute_request(req))
  response_parser.parse_response(resp)
}
...
pub fn execute_put(
  ...
  use req <- result.try(request_builder.build_put_request(config, path, body))
...
pub fn execute_patch(
  ...
  use req <- result.try(request_builder.build_patch_request(config, path, body))
