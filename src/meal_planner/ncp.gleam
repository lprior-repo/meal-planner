/// NCP (Nutrition Control Plane) functions for nutrition tracking and reconciliation
import gleam/float
import gleam/int
import gleam/list
import gleam/string
...
pub fn get_nutrition_history(days: Int) -> Result(List(NutritionState), String) {
  analysis.get_nutrition_history(days)
}
...
pub fn analyze_nutrition_trends(history: List(NutritionState)) -> TrendAnalysis {
  analysis.analyze_nutrition_trends(history)
}
...
pub fn get_default_goals() -> NutritionGoals {
  reconciliation.get_default_goals()
}
...
pub fn generate_adjustments(
  ...
  reconciliation.generate_adjustments(deviation, recipes, limit)
}
...
pub fn average_nutrition_history(history: List(NutritionState)) -> NutritionData {
  analysis.average_nutrition_history(history)
}
...
pub fn score_recipe_for_deviation(
  ...
  reconciliation.score_recipe_for_deviation(deviation, macros)
}
...
pub fn select_top_recipes(
  ...
  reconciliation.select_top_recipes(deviation, recipes, limit)
}
...
pub fn generate_reason(deviation: DeviationResult, macros: Macros) -> String {
  reconciliation.generate_reason(deviation, macros)
}
