...
//!
//! Removes a recipe entry and all its associated shopping list entries from a meal plan's shopping list.
//!
...
struct Input {
    ...
    /// Meal plan ID
    mealplan_id: i64,
    /// Recipe ID to delete from shopping list
    recipe_id: i64,
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client.delete_recipe_from_shopping_list(input.mealplan_id, input.recipe_id)?;
