...
fn run() -> anyhow::Result<Output> {
	...
	if parsed.updates.is_empty() {
		return Err(anyhow::anyhow!("updates array is required and must not be empty"));
	}

	let client = TandoorClient::new(&config)?;
	let result = client.batch_update_recipes(&parsed.updates)?;

	Ok(Output {
		success: true,
		updated_count: Some(result.updated_count),
		updated_ids: Some(result.updated_ids),
		error: None,
	})
}
...
mod tests {
	...
	fn test_output_serialization() {
		let output = Output {
			...
			updated_ids: Some(vec![1, 2]),
			error: None,
		};
		let json = serde_json::to_string(&output).unwrap();
		assert!(json.contains("\"success\":true"));
		assert!(json.contains("\"updated_count\":2"));
		assert!(json.contains("\"updated_ids\":[1,2]"));
	}
	...
	fn test_parse_windmill_input() {
		...
		assert_eq!(parsed.updates[0].id, 1);
	}
