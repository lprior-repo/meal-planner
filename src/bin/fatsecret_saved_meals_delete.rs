...
use meal_planner::fatsecret::saved_meals::{delete_saved_meal, SavedMealId};
...
struct Input {
    ...
    /// ID of the saved meal to delete
...
async fn run() -> Result<Output, Box<dyn std::error::Error>> {
    ...
    delete_saved_meal(&config, &token, &saved_meal_id).await?;
