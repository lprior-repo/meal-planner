...
use meal_planner::tandoor::{RecipeBook, TandoorClient, TandoorConfig, UpdateRecipeBookRequest};
...
fn run() -> anyhow::Result<Output> {
	...
	let recipe_book = client.update_recipe_book(parsed.id, &request)?;
...
mod tests {
	...
	fn test_input_parsing() {
		let json = r#"{"tandoor": {"base_url": "http://localhost:8090", "api_token": "test"}, "id": 1, "name": "Updated"}"#;
		let parsed: Input = serde_json::from_str(json).unwrap();
		assert_eq!(parsed.id, 1);
		assert_eq!(parsed.name, Some("Updated".to_string()));
	}

	#[test]
	fn test_input_parsing_partial_update() {
		let json = r#"{"tandoor": {"base_url": "http://localhost:8090", "api_token": "test"}, "id": 1}"#;
		let parsed: Input = serde_json::from_str(json).unwrap();
		assert_eq!(parsed.id, 1);
