...
//!
//! Retrieves all shopping list entries for a given meal plan.
//!
//! JSON input (CLI arg or stdin):
//!   `{"tandoor": {...}, "mealplan_id": 123}`
//!
...
fn run() -> Result<Vec<ShoppingListEntry>, Box<dyn std::error::Error>> {
    ...
    let entries = client.list_shopping_list_entries(input.mealplan_id)?;
...
mod tests {
    ...
    fn test_input_deserialization() {
        let json = r#"{"tandoor": {"base_url": "http://localhost:8090", "api_token": "test"}, "mealplan_id": 123}"#;
        let input: Input = serde_json::from_str(json).expect("Failed to parse test JSON");
        assert_eq!(input.mealplan_id, 123);
        assert_eq!(input.tandoor.api_token, "test");
    }
