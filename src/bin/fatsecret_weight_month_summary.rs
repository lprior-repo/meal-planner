...
//!
//! Retrieves aggregated weight statistics for a specific month.
...
use meal_planner::fatsecret::weight::get_weight_month_summary;
...
async fn run() -> Result<Output, Box<dyn std::error::Error>> {
    ...
    let summary = get_weight_month_summary(&config, &token, input.date_int).await?;
