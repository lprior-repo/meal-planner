...
struct Input {
    ...
    /// Any day in the target month (days since epoch)
    date_int: i32,
}
...
async fn run() -> Result<Output, Box<dyn std::error::Error>> {
    ...
    // Get month summary
    let month = get_month_summary(&config, &token, input.date_int).await?;

    Ok(Output {
        success: true,
        month: serde_json::to_value(month)?,
    })
}
