...
//!
//! Adds a recipe to user's favorites list.
...
use meal_planner::fatsecret::favorites::add_favorite_recipe;
...
struct Input {
    ...
    /// ID of recipe to add to favorites
    recipe_id: String,
}
...
async fn run() -> Result<Output, Box<dyn std::error::Error>> {
    ...
    add_favorite_recipe(&config, &token, &input.recipe_id).await?;
