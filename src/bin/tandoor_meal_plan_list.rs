//! List meal plans from Tandoor
//!
//! Lists all meal plans with optional pagination support.
//!
//! JSON input (CLI arg or stdin):
//!   `{"tandoor": {...}, "page": 1, "page_size": 10}`
//!
//! JSON stdout:
//!   `{"success": true, "count": 5, "meal_plans": [...]}`
//!   `{"success": false, "error": "..."}`

// CLI binaries: exit and unwrap/expect are acceptable at the top level
#![allow(clippy::exit, clippy::unwrap_used, clippy::expect_used)]

use meal_planner::tandoor::{TandoorClient, TandoorConfig};
use serde::{Deserialize, Serialize};
use std::io::{self, Read};

#[derive(Deserialize)]
struct Input {
    /// Tandoor configuration
    tandoor: TandoorConfig,
    /// Optional page number
    #[serde(default)]
    page: Option<u32>,
    /// Optional page size
    #[serde(default)]
    page_size: Option<u32>,
}

#[derive(Serialize)]
struct Output {
    success: bool,
    #[serde(skip_serializing_if = "Option::is_none")]
    count: Option<i64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    meal_plans: Option<Vec<serde_json::Value>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    error: Option<String>,
}

fn main() {
    match run() {
        Ok(output) => {
            println!(
                "{}",
                serde_json::to_string(&output).expect("Failed to serialize output JSON")
            );
        }
        Err(e) => {
            let error = Output {
                success: false,
                count: None,
                meal_plans: None,
                error: Some(e.to_string()),
            };
            println!(
                "{}",
                serde_json::to_string(&error).expect("Failed to serialize error JSON")
            );
            std::process::exit(1);
        }
    }
}

fn run() -> anyhow::Result<Output> {
    // Read input: prefer CLI arg, fall back to stdin
    let input: Input = if let Some(arg) = std::env::args().nth(1) {
        serde_json::from_str(&arg)?
    } else {
        let mut input_str = String::new();
        io::stdin().read_to_string(&mut input_str)?;
        serde_json::from_str(&input_str)?
    };

    let client = TandoorClient::new(&input.tandoor)?;

    // List meal plans
    let result = client.list_meal_plans(input.page, input.page_size)?;

    Ok(Output {
        success: true,
        count: Some(result.count),
        meal_plans: Some(
            result
                .results
                .iter()
                .map(|mp| serde_json::to_value(mp).unwrap_or_else(|_| serde_json::json!({})))
                .collect(),
        ),
        error: None,
    })
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_input_parsing_minimal() {
        let json = r#"{"tandoor": {"base_url": "http://localhost:8090", "api_token": "test"}}"#;
        let input: Input = serde_json::from_str(json).expect("Failed to parse test JSON");
        assert_eq!(input.page, None);
        assert_eq!(input.page_size, None);
    }

    #[test]
    fn test_input_parsing_with_pagination() {
        let json = r#"{"tandoor": {"base_url": "http://localhost:8090", "api_token": "test"}, "page": 2, "page_size": 20}"#;
        let input: Input = serde_json::from_str(json).expect("Failed to parse test JSON");
        assert_eq!(input.page, Some(2));
        assert_eq!(input.page_size, Some(20));
    }

    #[test]
    fn test_output_serialization() {
        let output = Output {
            success: true,
            count: Some(5),
            meal_plans: Some(vec![serde_json::json!({"id": 1})]),
            error: None,
        };
        let json = serde_json::to_string(&output).expect("Failed to serialize output JSON");
        assert!(json.contains("\"success\":true"));
        assert!(json.contains("\"count\":5"));
    }

    #[test]
    fn test_error_output() {
        let output = Output {
            success: false,
            count: None,
            meal_plans: None,
            error: Some("API error".to_string()),
        };
        let json = serde_json::to_string(&output).expect("Failed to serialize output JSON");
        assert!(json.contains("\"success\":false"));
        assert!(json.contains("API error"));
        assert!(!json.contains("\"count\""));
    }
}
