...
//!   `{"tandoor": {...}, "id": 123, "name": "kg", "plural_name": "kilograms"}`
//!
...
use meal_planner::tandoor::{TandoorClient, TandoorConfig, UpdateUnitRequest};
...
struct Input {
    ...
    /// Unit ID to update
    id: i64,
    /// New unit name (optional)
    #[serde(default)]
    name: Option<String>,
    /// New plural name (optional)
    #[serde(default)]
...
fn run() -> anyhow::Result<Output> {
    ...
    let request = UpdateUnitRequest {
        name: input.name,
        plural_name: input.plural_name,
    };

    let updated = client.update_unit(input.id, &request)?;
