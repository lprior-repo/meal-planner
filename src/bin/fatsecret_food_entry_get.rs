//! Get FatSecret food diary entry
//!
//! Retrieves a specific food diary entry by ID.
...
use meal_planner::fatsecret::diary::{get_food_entry, FoodEntryId};
...
async fn run() -> Result<Output, Box<dyn std::error::Error>> {
    ...
    let entry = get_food_entry(&config, &token, &entry_id).await?;
