...
pub fn benchmark_scaling_behavior_test() {
  ...
  io.println(
    "- If time grows quadratically: O(n²) - pair generation bottleneck",
  )
...
pub fn performance_report_template_test() {
  ...
  io.println(
    "   - Filter recipes by calorie range before combination generation",
  )
  io.println(
    "   - Example: If target=2000 cal, ignore recipes >1500 cal (can't combine)",
  )
