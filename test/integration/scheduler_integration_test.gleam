...
pub fn job_manager_to_daily_advisor_integration_test() {
  case postgres.config_from_env() {
    ...
    Ok(config) -> {
      ...
      let job_id = id.job_id("test_daily_advisor_" <> birl.now_iso8601())
      let job =
        ScheduledJob(
          id: job_id,
...
pub fn job_manager_to_weekly_trends_integration_test() {
  case postgres.config_from_env() {
    ...
    Ok(config) -> {
      ...
      let job_id = id.job_id("test_weekly_trends_" <> birl.now_iso8601())
      let job =
        ScheduledJob(
          id: job_id,
...
pub fn executor_retry_on_transient_failure_test() {
  case postgres.config_from_env() {
    ...
    Ok(_config) -> {
      ...
      let job_id = id.job_id("test_retry_" <> birl.now_iso8601())
      let job =
        ScheduledJob(
          id: job_id,
...
pub fn job_execution_duration_tracking_test() {
  case postgres.config_from_env() {
    ...
    Ok(_config) -> {
      let job =
        ScheduledJob(
          id: id.job_id("test_duration_" <> birl.now_iso8601()),
          job_type: AutoSync,
          frequency: types.Once,
          status: Pending,
          priority: High,
