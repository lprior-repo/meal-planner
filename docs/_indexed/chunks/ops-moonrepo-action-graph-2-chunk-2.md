---
doc_id: ops/moonrepo/action-graph-2
chunk_id: ops/moonrepo/action-graph-2#chunk-2
heading_path: ["Action graph", "Actions"]
chunk_type: prose
tokens: 757
summary: "Actions"
---
...
### Setup environment (v1.35.0)

This action runs after the toolchain has been setup, but before dependencies are installed, so that the development environment can be setup and configured. This includes operations such as modifying a manifest (`package.json`, etc), updating configuration files, initializing venv's (Python), so on and so forth.
