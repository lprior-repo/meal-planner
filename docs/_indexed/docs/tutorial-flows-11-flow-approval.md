...
category: tutorial
tags: ["flows", "suspend", "beginner", "tutorial"]
---
...
Use `wmill.getResumeUrls()` in [TypeScript](./meta-1_typescript_quickstart-index.md) or `wmill.get_resume_urls()` in [Python](./meta-2_python_quickstart-index.md) from the [wmill client](./ops-2_clients-ts-client.md) to generate secret URLs.
...
The approval form argument values can be accessed in the subsequent step by [connecting](./tutorial-flows-16-architecture.md#input-transform) input fields to either `resume["argument_name"]` for a specific argument, or simply `resume` to obtain the complete payload.
...
Then another user ([operator](./meta-16_roles_and_permissions-index.md#operator), since is only "Viewer" in the [folder](./meta-8_groups_and_folders-index.md) of the flow), runs the flow and sees the prompt automatically shown when running the flow.
...
You can add a description to give clear instructions that support the whole range of [rich display rendering](./meta-19_rich_display_rendering-index.md) (including markdown).
...
By enabling this option, only logged in users who are members of the specified [group](./meta-8_groups_and_folders-index.md) can approve the step.
...
To get the list of users, just have the step after the approval step return the `approvers` key. For example by taking an input [connected](./tutorial-flows-16-architecture.md) to the `approvers` key.
...
The Windmill [Python](./ops-2_clients-python-client.md) and [TypeScript](./ops-2_clients-ts-client.md) clients both have a helper function to request an interactive approval on Slack. An interactive approval is a Slack message that can be approved or rejected directly from Slack without having to go back to the Windmill UI.
