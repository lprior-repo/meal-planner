...
Note that errors have a [specific shape](./meta-19_rich_display_rendering-index.md#error).
...
When enabled, the flow will continue to the next step after going through all the retries (if any) even if this step fails. This enables to process the error in a [branch one](./concept-flows-13-flow-branches.md) for instance.
