...
title: "HTTP routes"
category: meta
tags: ["windmill", "39_http_routing", "http", "advanced", "meta"]
---

# HTTP routes

> **Context**: Windmill supports HTTP Routes as triggers to execute runnables (scripts or flows) whenever the route is hit by an external HTTP request, and it can al
...
HTTP routes support local error handlers that override workspace error handlers for specific routes. See the [error handling documentation](./meta-10_error_handling-index.md#trigger-error-handlers) for configuration details and examples.

## See Also

- [Admins workspace](../../advanced/18_instance_settings/index.mdx#admins-workspace)
