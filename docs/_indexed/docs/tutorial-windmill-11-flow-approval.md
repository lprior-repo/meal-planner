...
Note that approval steps can be applied the same configurations as regular steps ([Retries](./ops-windmill-14-retries.md), [Early stop/Break](./concept-windmill-2-early-stop.md) or [Suspend](./tutorial-windmill-15-sleep.md)).
...
The Windmill [TypeScript](./ops-windmill-ts-client.md) client exposes helper functions to request a approval on Microsoft Teams. The interactive approval is a Teams message that can be approved or rejected directly from Teams without having to go back to the Windmill UI where as the non-interactive approval will be a simple link that will open the approval page in the Windmill UI in your browser.
...
3. The answer is a condition to [branches](./concept-windmill-13-flow-branches.md) that lead either a refund, a refusal or a deeper investigation.
...
For the sake of the example, we made this flow simple with a [manual trigger](./meta-windmill-index-99.md). Two input were used: "User email" and "Order number", both strings.
...
Those are the keys you can use as predicate expressions for your [branches](./concept-windmill-13-flow-branches.md).

![Branches predicate expressions](../assets/flows/tuto_approval_branches.png.webp 'Branches predicate expressions')

> With [Branch one](./concept-windmill-13-flow-branches.md#branch-one), the first branch whose predicate expression is `true` will execute.
