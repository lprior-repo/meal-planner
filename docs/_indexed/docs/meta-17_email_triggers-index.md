...
category: meta
tags: ["advanced", "email", "17_email_triggers", "meta"]
---

# Email triggers

> **Context**: Scripts and flows can be triggered by email messages sent to a specific email address, leveraging [SMTP](https://www.cloudflare.com/learning/email-sec
...
Attchments are uploaded to the [workspace object storage (s3)](./meta-38_object_storage_in_windmill-index.md#workspace-object-storage) and are replaced in `parsed_email` by s3 objects (`{ s3: "path/to/key" }`).
...
From a script or flow [deployed](./meta-0_draft_and_deploy-index.md) page, you will find on the "Details & Triggers" - "Email" tab the email address to use.

![Trigger panel](./trigger_panel.png "Trigger panel")

## Error handling

Email triggers support local error handlers that override workspace error handlers for specific triggers. See the [error handling documentation](./meta-10_error_handling-index.md#trigger-error-handlers) for configuration details and examples.
...
- [Email triggers from cloud](./email_triggers_cloud.png "Email triggers from cloud")
- [instance settings](../18_instance_settings/index.mdx#email-domain)
