...
> **Context**: Dagger's `LLM` type includes API methods to attach objects to a Large Language Model (LLM), send prompts, and receive responses.
...
Dagger [modules](./ops-features-reusability.md) are collections of Dagger Functions. When you give a Dagger module to the `LLM` type, every Dagger Function is turned into a tool that the LLM can call.
