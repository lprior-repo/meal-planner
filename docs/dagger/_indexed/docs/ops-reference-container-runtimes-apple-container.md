---
id: ops/reference/container-runtimes-apple-container
title: "Apple Container Runtime"
category: ops
tags: ["ops", "github", "git", "ai", "container"]
---

# Apple Container Runtime

> **Context**: Dagger can be used with any OCI-compatible container runtime, including [`container` from Apple](https://github.com/apple/container).


Dagger can be used with any OCI-compatible container runtime, including [`container` from Apple](https://github.com/apple/container).

> **Warning**: The support for Apple's `container` is still early and experimental.

## Prerequisites

Ensure that `container` is installed on your Mac:

```bash
container system start
```

Install `container` using the official instructions at [github.com/apple/container](https://github.com/apple/container).

## Example

```bash
$ container ls
ID  IMAGE  OS  ARCH  STATE  ADDR  CPUS  MEMORY

$ dagger core version
v0.18.19

$ container ls
ID                      IMAGE                               OS     ARCH   STATE    ADDR          CPUS  MEMORY
dagger-engine-v0.18.19  registry.dagger.io/engine:v0.18.19  linux  arm64  running  192.168.64.4  4     8192 MB
```

## About Apple's `container`

Apple's [`container`](https://github.com/apple/container) is a tool for creating and running Linux containers using lightweight virtual machines on a Mac. It is written in Swift and optimized for Apple silicon.

## Resources

Join the [Discord](https://discord.gg/dagger-io) and ask questions in the [#macos channel](https://discord.com/channels/707636530424053791/1155969163282628639).

## See Also

- [Documentation Overview](./COMPASS.md)
