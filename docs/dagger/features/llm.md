...
The documentation for the modules are provided to the LLM via [inline documentation](/extending/documentation) in your Dagger Functions. The LLM can then analyze the available tools and select which ones to use.
...
Dagger provides [end-to-end tracing](/features/observability) of prompts, tool calls, and even low-level system operations. All agent state changes are observable in real time.
