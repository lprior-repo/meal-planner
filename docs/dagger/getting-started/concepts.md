...
In addition to supporting basic types (string, boolean, integer, array...), the Dagger API also provides [powerful types](/getting-started/types) for working with workflows, such as `Container`, `GitRepository`, `Directory`, `Service`, `Secret`. You can use these types as arguments to Dagger functions.
...
Functions can be chained with the CLI, or programmatically in a [custom Dagger function](/extending/functions) using a Dagger SDK.

## Modules

Dagger [modules](/extending/modules) are collections of Dagger Functions, packaged together for easy sharing and consumption. They are portable and reusable across languages.
...
The Dagger API is extensible and shareable by design. You can extend the API by creating Dagger [modules](/extending/modules). You are encouraged to write your own Dagger modules and share them with others.
