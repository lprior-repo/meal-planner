...
Dagger can use [remote repositories](/extending/remote-repositories) as Dagger [modules](/features/reusability). This feature is compatible with all major Git hosting platforms such as GitHub, GitLab, BitBucket, Azure DevOps, Codeberg, and Sourcehut. Dagger supports authentication via both HTTPS (using Git credential managers) and SSH (using a unified authentication approach).
...
Here is the same example using SSH authentication. Note that this requires [SSH authentication to be properly configured](/extending/remote-repositories#ssh-authentication) on your Dagger host.
...
For more information, refer to the documentation on [remote repository access](/extending/remote-repositories).
