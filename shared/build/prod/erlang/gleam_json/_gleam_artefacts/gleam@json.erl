...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(DLD)) -> {ok, DLD} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(DKX)) -> {ok, DKX} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(DKT)) -> {ok, DKT} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(DLJ), fun((DLJ) -> json())) -> json().
...
-spec array(list(DLN), fun((DLN) -> json())) -> json().
