...
-spec be_ok({ok, DRB} | {error, any()}) -> DRB.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, DRG}) -> DRG.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(DRJ)) -> DRJ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
